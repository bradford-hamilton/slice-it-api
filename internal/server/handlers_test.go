...
func TestAPI_create(t *testing.T) {
	...
	req, err := http.NewRequest("POST", "/api/v1/urls", bytes.NewBuffer([]byte(`{ "long": "https://www.reddit.com/r/golang/comments/is81vi/just_because_you_can_doesnt_mean_you_should" }`)))
	if err != nil {
		t.Fatal(err)
	}
	...
	// `{ "shortURL\": "043fdd58" }`
	expected := []byte{34, 123, 32, 92, 34, 115, 104, 111, 114, 116, 85, 82, 76, 92, 34, 58, 32, 48, 52, 51, 102, 100, 100, 53, 56, 32, 125, 34, 10}
	if !bytes.Equal(rr.Body.Bytes(), expected) {
		t.Errorf("handler returned unexpected body: got %v want %v", rr.Body.Bytes(), expected)
	}
}
